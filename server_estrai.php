<?php
    ...
    if(isset($idserver)) {
        ...
        if(mysqli_num_rows($query) == 1) {
            ...
            $query = mysqli_query($dbh, "select n.idnumero
                                         from ".PREFIX."numero n left join (select e.idnumero
                                                                            from ".PREFIX."estrarre e
                                                                            where e.idserver = '$idserver')t1
                                         on n.idnumero = t1.idnumero
                                         where t1.idnumero is null");
            $numeri = array();
            ...
            $query = mysqli_query($dbh, "select p.idpremio, p.testo
                                         from ".PREFIX."premio p left join (select v.idpremio
                                                                            from ".PREFIX."utente u, ".PREFIX."server s, ".PREFIX."vincere v
                                                                            where u.idserver = s.idserver
                                                                            and v.idutente = u.idutente
                                                                            and s.idserver = $idserver
                                                                            group by v.idpremio)t1
                                         on t1.idpremio = p.idpremio
                                         where t1.idpremio is null
                                         order by p.idpremio asc
                                         limit 1;");
            ...
            if($next <= 5) {    // Fino alla cinquina
                ...
                if($query = mysqli_fetch_array(mysqli_query($dbh, "select u.idutente, n.cartella, n.riga
                                                                   from ".PREFIX."utente u, ".PREFIX."server s, ".PREFIX."estrarre e, ".PREFIX."numero n
                                                                   where u.idserver = s.idserver
                                                                   and e.idserver = s.idserver
                                                                   and e.idnumero = n.idnumero
                                                                   and u.nick = 'Tombolone'
                                                                   and s.idserver = $idserver
                                                                   group by n.cartella, n.riga
                                                                   having count(n.idnumero) = $next;"))) {
                ...
                $query = mysqli_query($dbh, "select u.idutente, u.nick
                                             from ".PREFIX."utente u, ".PREFIX."server s,
                                                  ".PREFIX."avere a, ".PREFIX."cartella c,
                                                  ".PREFIX."comporre k, ".PREFIX."numero n,
                                                  ".PREFIX."estrarre e
                                             where u.idserver = s.idserver
                                             and a.idutente = u.idutente
                                             and a.idcartella = c.idcartella
                                             and k.idcartella = c.idcartella
                                             and k.idnumero = n.idnumero
                                             and e.idnumero = n.idnumero
                                             and e.idserver = s.idserver
                                             and u.uscito is null
                                             and s.idserver = $idserver
                                             group by u.idutente, k.riga
                                             having count(n.idnumero) = $next");
